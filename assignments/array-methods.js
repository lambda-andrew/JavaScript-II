...
runners.forEach(function(item, array){
...
const bigLetters = runners.map(function(currentValue){
  return currentValue.first_name.toUpperCase();
});

allCaps.push(bigLetters);
console.log(allCaps);
...
const bigShirts = runners.filter(function(shirts){
    return shirts.shirt_size === "L";
});

largeShirts.push(bigShirts)
...
const newTicket = runners.reduce(function(accumulator, currentValue){
  return accumulator + currentValue.donation;
}, 0);

ticketPriceTotal.push(newTicket);
...
const newMoney = runners.map(function(currentValue){
  return currentValue.donation + 100;
})

extraDonations.push(newMoney);

console.log(extraDonations);

// Problem 2
const contactRunners = [];

runners.forEach(function(item){
  contactRunners.push(item.email);
})
console.log(contactRunners);
// Problem 3

const noSmalls = [];

const smallShirts = runners.filter(function(shirts){
return shirts.shirt_size !== "XS";
});

noSmalls.push(smallShirts);
